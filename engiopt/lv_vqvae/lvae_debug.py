...
SUMMARY OF ALL CHANGES FROM LV-VQVAE TO MATCH TRUE LVAE AS CLOSELY AS POSSIBLE:
1) Change decoder to LVAE decoder
2) Change encoder to LVAE encoder
3) Change desired input size to 100 instead of 128 to comply with the above hard-coded encoder/decoder
4) Change NMAE to NMSE and match threshold value of 0.05
5) Match Adam optimizer to only use encoder and decoder params (remove quant_conv and post_quant_conv for use_vq=False)
6) Change LR from 2e-4 to 1e-4
7) Change default latent_dim from 256 to 100
8) Change batch_size_vqvae from 16 to 128
9) Change lv_constraint_mode default from gradient_balanced to one_sided
10) Change lv_eta from 1e-4 to 0 (LVAE default)
11) Change conditional argument from True to False (not strictly necessary but removes CVQVAE training)
12) Revert the update_moving_mean function and loss_vol (in the form of loss_vol_new) to be more similar to the original LVAE version (2 dimensional input rather than 4 dimensional)
13) Remove image resizing in pre-processing and allow it to stay at (50,100) while the encoder transforms it to (100,100) per original LVAE. Because of this had to re-include unsqueeze in training_ds["optimal_design"][:].unsqueeze(1)

TAKEAWAYS:
1) Increase batch size from 16 to 128
2) Revert back to NMSE from NMAE in general as NMSE produces more smoothed designs which can be helpful for warm-starting. However, keep the option to use either NMAE or NMSE and still calculate/log both.
3) Reminder to start plotting ground truths next to generated samples in the validation run tracked on wandb
4) ***KEY POINT*** The VQVAE-based LVAE was likely pruning down to 1 dim because of the feature map size (4-dimensional input), leading to the effective dimension being the feature map size (i.e., 1 "pruned dim" x 7 x 7 = 49 effective dims)
    - Introduce a new argument to more easily tune the feature map size via encoder and decoder depth, i.e., args.reduction_factor = 16 means log2(16) = 4 downsamplings, which is the default and yields Bx1x100x100 --> BxDx7x7 latent map
    - Presents a key issue with LV-VQVAE in its current state: model cannot automatically adapt or "prune" the feature map size in any way. It is a hyperparam that needs to be tuned.
    - Addressing this would entail moving away from the standard conv-based encoder and decoder
"""

#  B x 1 x 100 x 100 --> B x D x 7 x 7 --> B x D x 7 x 7 --> B x 1 x 100 x 100
#  NOTE FROM MARK: VERIFY how/why exactly the VQVAE-based LVAE is able to prune to 1 dimension. Does it make sense that extra info is able to be stored in the 7x7 feature map if the decoder is purely convolutional and only the encoder has access to attention layers?
...
class Args:
    ...
    n_epochs_vqvae: int = 10000
    """number of epochs of training"""
    batch_size_vqvae: int = 128
    ...
    lv_pruning_params: dict[str, Any] | None = field(default_factory=lambda: {"threshold": 0.05, "beta": 0.9, "alpha": 0.0})  # threshold: 0.02 default for plummet; 0.25 more aggressive
    ...
    lv_constraint_mode: str = "one_sided"
    ...
    lv_decoder_lipschitz_scale: float = 1.0
...
def loss_vol_new(z: th.Tensor, *, active_mask: th.Tensor, frozen_std: th.Tensor, eta: float) -> th.Tensor:
    ...
    s = frozen_std.clone()
    if active_mask.any():
        s[active_mask] = z[:, active_mask].std(0)
...
class TrueSNDecoder2D(nn.Module):
    ...
    def forward(self, z: th.Tensor) -> th.Tensor:
        ...
        return th.sigmoid(x * self.lipschitz_scale)  # Scale then squash to [0, 1]
...
class VQVAE(nn.Module):
    ...
    def __init__(  # noqa: PLR0913
        ...
        *,
        ...
        active_latent_dim = cond_latent_dim if is_c else latent_dim

        if self.use_vq:
            active_codebook_vectors = cond_codebook_vectors if is_c else num_codebook_vectors
            self.codebook = Codebook(
                num_codebook_vectors=active_codebook_vectors,
                latent_dim=active_latent_dim
            )

        if is_c:
            self.encoder = CondEncoder(cond_feature_map_dim, cond_dim, cond_hidden_dim, cond_latent_dim)
            self.decoder = TrueSNCondDecoder(cond_latent_dim, cond_dim, cond_hidden_dim, cond_feature_map_dim)
        elif self.use_vq:
            self.encoder = Encoder(
                encoder_channels, encoder_start_resolution, encoder_attn_resolutions,
                encoder_num_res_blocks, image_channels, latent_dim
            )
            self.decoder = TrueSNDecoder(
                decoder_channels, decoder_start_resolution, decoder_num_res_blocks,
                image_channels, latent_dim
            )
        else:
            self.encoder = Encoder2D(latent_dim, design_shape, args.resize_dimensions)
            self.decoder = TrueSNDecoder2D(latent_dim, design_shape, lipschitz_scale=args.lv_decoder_lipschitz_scale)

        if is_c or self.use_vq:
            self.quant_conv = nn.Conv2d(active_latent_dim, active_latent_dim, kernel_size=1)
            self.post_quant_conv = nn.Conv2d(active_latent_dim, active_latent_dim, kernel_size=1)
        else:
    ...
    def apply_pruning(self, z, active_mask, frozen_mean):
        ...
        if not pruned.any():
            return z
        z = z.clone()
        z[:, pruned] = frozen_mean[pruned].view(1, -1)
        return z
    ...
    def update_moving_mean(
        ...
        if zstd is None or zmean is None:
            ...
            zstd = th.lerp(zstd, z.std(0), 1 - beta)
            zmean = th.lerp(zmean, z.mean(0), 1 - beta)
...
if __name__ == "__main__":
    ...
    data_var = set_data_variance(training_ds["optimal_design"][:].unsqueeze(1))
    data_std = th.sqrt(data_var)

    # Move to device only here
    th_training_ds = th.utils.data.TensorDataset(
        th.as_tensor(training_ds["optimal_design"][:].unsqueeze(1)).to(device),
    ...
    th_val_ds = th.utils.data.TensorDataset(
        th.as_tensor(val_ds["optimal_design"][:].unsqueeze(1)).to(device),
    ...
    for epoch in tqdm.trange(args.n_epochs_vqvae):
        for i, data in enumerate(dataloader_vqvae):
            ...
            l2_loss = f.mse_loss(designs, decoded_images)
            nmae = rec_loss / data_std
            nmse = l2_loss / data_var
