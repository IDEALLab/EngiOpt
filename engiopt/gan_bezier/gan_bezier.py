...
class Args:
    ...
    generator_checkpoint_path: str = "bezier_generator.pth"
    """Final generator checkpoint path used when save_model is enabled."""
    discriminator_checkpoint_path: str = "bezier_discriminator.pth"
        ...
        for i, real_designs_cpu in enumerate(dataloader):
            ...
            if args.save_model and epoch == args.n_epochs - 1 and i == len(dataloader) - 1:
                ...
                if args.track:
                    ...
                    artifact_gen.add_file(args.generator_checkpoint_path, name="bezier_generator.pth")
                    artifact_disc = wandb.Artifact(f"{args.problem_id}_{args.algo}_discriminator", type="model")
                    artifact_disc.add_file(args.discriminator_checkpoint_path, name="bezier_discriminator.pth")
