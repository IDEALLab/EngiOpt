...
class Args:
    ...
    """Optional local checkpoint directory containing cvqgan.pth, vqgan.pth, transformer.pth."""
...
if __name__ == "__main__":
    ...
    if args.checkpoint_path is not None:
        ckpt_path_cvqgan = os.path.join(args.checkpoint_path, "cvqgan.pth")
        ckpt_path_vqgan = os.path.join(args.checkpoint_path, "vqgan.pth")
        ckpt_path_transformer = os.path.join(args.checkpoint_path, "transformer.pth")
        ...
        run_config: dict[str, Any] = dict(ckpt_transformer.get("args", {}))
        if not run_config:
            raise ValueError("Local VQGAN eval requires checkpoint args metadata for model reconstruction.")
    else:
