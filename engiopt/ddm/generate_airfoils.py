...
def test_bae_reconstruction(bae_model, device, save_dir=None, n_wings=20):
    ...
    test_items = list(problem.dataset["test"])
...
def check_training_latent_distribution(bae_model, device, n_samples=200):
    ...
    problem = Wings3D(seed=0)
    train_items = list(problem.dataset["train"])[:n_samples]

    # Collect latents per slice: list of 9 lists
    slice_latents = [[] for _ in range(9)]
    ...
    overall = torch.stack([stats[s]['tensor'] for s in range(9)])
...
def compare_latent_distributions(train_stats, generated_airfoils, save_dir):
    ...
    generated_airfoils: [B, 9, 3, 30] — the raw DDM output latents
    """
    print("\n" + "="*60)
    print("TRAINING vs GENERATED LATENT DISTRIBUTION COMPARISON")
    print("="*60)

    gen = generated_airfoils.cpu()  # [B,9,3,30]
...
def check_channel0_validity(bae_model, device):
    ...
    problem = Wings3D(seed=0)
    train_items = list(problem.dataset["train"])
    ...
    with torch.no_grad():
        for item in train_items:
            ...
            ch0_vals = torch.cat([z[:, 0, :] for z in wing_latents])  # [9*30]
            ...
            if has_below:
                ...
                decoded = torch.stack(decoded_slices, dim=0)  # [9, 2, N]
...
def _decode_latents(generated_airfoils, bae_model):
    """Decode a batch of DDM latents [B,9,3,L] → airfoil coords [B,9,2,N]."""
    decoded_slices = []
    with torch.no_grad():
        for s in range(9):
    ...
    return torch.stack(decoded_slices, dim=1)  # [B, 9, 2, N]
...
def generate_airfoils_direct(ddm_model, bae_model, num_samples=10, device='cpu',
    ...
    for attempt in range(max_attempts):
        ...
        noise_x     = torch.randn(batch_n, 9, 3, 30).to(device)
...
def save_comparison_plots(generated_airfoil, ground_truth_coords, alpha_gen, alpha_gt, save_dir, wing_idx,
    ...
    generated_airfoil: [9, 2, 192] - centered coordinates (trailing edge at y=0)
    ground_truth_coords: [9, 192, 2] - raw from dataset (trailing edge at original y)
    ...
    gt_np = ground_truth_coords.copy()  # Make a copy to avoid modifying original
    
    # Center the ground truth coordinates to match the generated format
    for s in range(9):
        # Subtract trailing edge y-coordinate (first point's y)
        gt_np[s, :, 1] -= gt_np[s, 0, 1]
        # Center the x-coordinate as well (trailing edge x becomes 1.0)
...
def save_slice_plots(generated_airfoil, alpha_value, save_dir, wing_idx):
    """
    Save a 3x3 grid of 2D slice plots for one generated wing.
    generated_airfoil: [9, 2, 192]
    """
...
def main():
    ...
    cfg = Config()
    # model_name and save_dir come from Config defaults (ddm_large_v1 / results/ddm)
    # To evaluate other models, override here:
    # cfg.model_name = "ddm_wingTL_changes"
    # cfg.model_name = "ddm_aoa_init_3d_final_v15_smoothreg"
    # cfg.model_name = "ddm_v5wingTL"
    ...
    from engibench.problems.wings3D.v0 import Wings3D
    problem = Wings3D(seed=0)
    item = list(problem.dataset["train"])[0]
