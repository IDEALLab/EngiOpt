...
class Config:
    ...
    # DDM / UNet
    w_dim: int = 15              # number of spanwise slices (new dataset has 15, old had 9)
    ...
    # Outputs
    save_dir: str = "results/ddm_newdata"
    model_name: str = "ddm_newdata_v1"
...
def main():
    ...
    if args.n_samples is not None:
        cfg.model_name = f"ddm_newdata_ablation_n{args.n_samples}_s{args.seed}"
        cfg.save_dir = f"results/ablation_newdata/n{args.n_samples}_s{args.seed}"
    ...
    # 2) Load new dataset (scan once, split into initial and final)
    ...
    print(f"Loaded new dataset train split: {len(base_dataset)} final items, "
