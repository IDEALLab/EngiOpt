...
from engiopt.ddm.train_ddm_newdata import Config, build_sampler, build_unet, load_bae, _SLICES_PKL, _SCALARS_PKL
...
def compute_metrics_for_pass(generated, gt_airfoils, gen_aoas, gt_aoas):
    """
    generated:   [N, w_dim, 2, 192]
    gt_airfoils: [N, w_dim, 2, 192]
    gen_aoas:    [N]
    gt_aoas:     [N]
    """
...
def generate_batch(ddm_model, bae_model, encoded_inits_batch, params_batch, device):
    ...
    noise_x     = torch.randn(B, w_dim, 3, 30, device=device)
    ...
    with torch.no_grad():
        ...
        decoded_slices = []
        for s in range(gen_z_raw.shape[1]):
        ...
        decoded = torch.stack(decoded_slices, dim=1)  # [B, w_dim, 2, 192]
...
def main():
    ...
    # Load test split
    dataset = NewWingsDataset(_SLICES_PKL, _SCALARS_PKL, seed=cfg.seed)
    test_dataset = [item for item in dataset["test"] if item["final"] == 1]
    initial_by_case = {item["case_num"]: item for item in dataset["test"] if item["initial"] == 1}
    ...
    for i in range(n_test):
        ...
        x_opt = torch.stack(x_opt_slices, dim=0)  # [w_dim, 2, 192]
