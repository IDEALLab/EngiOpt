...
class PrecomputedWingsDataset(Dataset):
    ...
    def __init__(self, z_opts, aoas, params, z_inits,
        ...
        self.z_opts        = z_opts        # [N, latent_channels, latent_length]
        self.aoas          = aoas          # [N, 1]
        self.params        = params        # [N, 4]
        self.z_inits       = z_inits       # [N, latent_channels, latent_length]
...
def precompute_latents(base_dataset, bae_model, device):
    ...
    This avoids re-encoding on every training step, which was the main
    bottleneck (~25 s/epoch → <1 s/epoch).
    """
    ...
    with torch.no_grad():
        for i, item in enumerate(base_dataset):
            ...
            x_opt  = coords[-1].permute(1, 0).unsqueeze(0).to(device)  # [1,2,192]

            z_init = bae_model.encode(
                x_init, return_z=True, z_ae_mode=True
            ).squeeze(0).cpu()   # [3,N]

            z_opt = bae_model.encode(
                x_opt, return_z=True, z_ae_mode=True
            ).squeeze(0).cpu()   # [3,N]
    ...
    return (
        torch.stack(z_opts_list),   # [N, 3, latent_length]
...
def build_sampler(cfg: Config):
    return samplers.BaselineSampler_AoA(
...
def train_one_epoch(ddm_model: DDM_AoAInit, loader: DataLoader,
...
def main():
    ...
    # 4) PRE-COMPUTE latents (the key fix — done once, not every epoch)
    ...
    unet = build_unet(cfg)
    print("Built UNet.")
    print(f"latent_channels_full =  latent_channels +  c_dim_latent +  x_dim_add +  alpha_dim")
